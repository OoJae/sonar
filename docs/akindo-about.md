...
2. **On-Chain Context:** Read current SSI index composition (MAG7, DEFI, MEME,
   USSI) on Base via viem multicall (`getTokenset()` plus ERC-20 metadata).
...
6. **Execution:** Valid theses are persisted and the book is rebalanced through
   ...
   for the index tokens). Every trade is stamped with its thesis id; flipping
   back to paper is an instant kill switch.

## Wave 1 Deliverables (Shipped)

* **Live Data Integration:** SoSoValue Open API on the High Frequency tier
  (100 rpm), with a rolling 100/min token bucket and per-endpoint TTL caching.
...
* **SSI Protocol Reader:** viem multicall against MAG7.ssi, DEFI.ssi, MEME.ssi,
  and USSI on Base.
* **Paper Trading Engine:** Drizzle-backed, mirroring the live SoDEX
...
* **Verifiable Track Record (`/track`):** Cumulative NAV-weighted return of the
  rebalanced book versus a buy-and-hold baseline of the same indices, per-thesis
  attribution, and win rate, every number linking to the dated, cited thesis
  behind it. Honest paper-plus-testnet framing.
* **Off-Chain NAV Computation:** Each index's on-chain tokenset priced against
  SoSoValue feeds (27 of 27 tokens), charted versus a buy-and-hold baseline on
  Portfolio and Track.
* **Macro Circuit Breaker:** Reading SoSoValue's macro events, the agent
  ...
  FOMC, NFP) is near, citing the event and persisting the reason.
...
* **Cross-Chain Funding (we declared Mirror bridging):** We declared a Mirror
  ...
  withdrawal, a real three-balance panel (Base USDC, ValueChain vUSDC, SoDEX
  venue) shows the cross-chain state, and Mirror ships as the config-gated
  mainnet bridge design.
* **Operational Hardening:** The production build runs under systemd with
  auto-restart; the nginx vhost is in the repo; TLS via Let's Encrypt.
